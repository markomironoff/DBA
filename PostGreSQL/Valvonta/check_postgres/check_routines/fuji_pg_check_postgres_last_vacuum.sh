...
##
##      -----------------------------------------------------------------------
##      $RCSfile: fuji_pg_check_postgres_last_vacuum.sh,v $
##      $Revision: 1.1 $
##
##      /**** FOR LINUX ****/
##
...
##
##      $Log: fuji_pg_check_postgres_last_vacuum.sh,v $
##
...
#
#
# Sijainti: /home/fujitsu/bin/check_postgres/fuji_pg_check_postgres_last_vacuum.sh
...
if [ -z $LAST_VACUUM_EXCLUDEDB ]
then
 LAST_VACUUM_EXCLUDEDB="xyz_huu_haa"
fi
$PSQL $PG_CONNECT_OPTS -l -A -F: 1>$TMPFILE 2>&1
DBS="`cat $TMPFILE | sed -ne "/:/ { /Name:Owner/d; /template0/d; s/:.*$//; p }" | egrep -v "$LAST_VACUUM_EXCLUDEDB"`"
for db in $DBS
do
$CHECK_POSTGRES_DIR/check_postgres_last_vacuum  --warning="$LAST_VACUUM_WARNING" --critical="$LAST_VACUUM_CRITICAL" --dbname=$db --perflimit=1 --exclude='pg_catalog.' | \
