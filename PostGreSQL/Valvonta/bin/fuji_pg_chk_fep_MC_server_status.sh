...
##
##      -----------------------------------------------------------------------
##      $RCSfile: fuji_pg_chk_fep_MC_server_status.sh,v $
##      $Revision: 1.0 $
##
##      /**** FOR LINUX ****/
##
##      Contents: Tarkistetaan Mirroring Controllerin hallitsemien servereiden
##      statukset
##
##      $Author: rosenjyr $
##      $Date: 2019/06/06 14:10:03 $
##
##      $Log: fuji_pg_chk_fep_MC_server_status.sh,v $
##
...
#
#
# Sijainti: /home/fujitsu/bin/fuji_pg_chk_fep_MC_server_status.sh
...
while read server_id host_role host host_status db_proc_status disk_status
do
  echo "$server_id $host_role $host $host_status $db_proc_status $disk_status" | egrep "abnormal|unknown"
  if [ $? -eq 0 ]
  then
    echo "1 ERROR in server $server_id status. Run mc_ctl status" > $TMPFILE
  else
    echo "0" > $TMPFILE
  fi
done < <(mc_ctl status | grep  -A5 "server_id" | tail -n 2)
...
then
  ...
  then
    cp $TMPFILE $WORKING_DIR/tmp/postgres_main_errors.out
    echo "MAJOR FEP_MIRRORIG_STATUS - problems in FEP instance mirroring" | \
    ${WORKING_DIR}/bin/collect_alarms.sh $fuji_progname_base
    /bin/rm -rf $TMPFILE
    exit '-1'
 fi # end if chk_if...
